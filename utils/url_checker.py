...
import tldextract
import utils.pythonwhois
# from bot.settings import BASE_DIR
import os

BASE_DIR = r"C:\Users\HP\Main Project"
...
class Checks():
    ...
    def WhoisCheck(self, domain, test=False):
        ...
        if creation_date is None:
            logging.info("Not able to extract creation date using python-whois library")
            # You may choose to handle this scenario differently based on your application's requirements
            # For example, you can log a warning or take some other action
            self.CHECKS['WhoisCheck']['status'] = 'FAIL'
            if test:
                TEST['status'] = 'FAIL'
            return TEST  # Or return None or raise an exception, depending on your logic
